...
function step!(net::NeuralNet, sec::Int, msec::Int)
    net.I = 13*(rand(N).-0.5)
    time = 1000*sec+msec
    trial = div(sec, 10)+1
    ...
    LTP!(net.STDP, net.sd, fired, msec, LTPinc) # time
    net.firings = vcat(net.firings, hcat(msec.*ones(length(fired)), fired))
    LTD!(net.STDP, net.sd, net.s, net.firings, net.I, msec, LTDinc) # time
    if sec%10==0
        if msec==1
    ...
    synweight_step!(net.sd, net.s, net.DA, msec, sm) # time
