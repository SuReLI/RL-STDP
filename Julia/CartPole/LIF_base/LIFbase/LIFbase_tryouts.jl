...
results = load_param("Julia/CartPole/LIF_base/results/results_HYP8888_vthresh.jld")
...
function sim(genes, render::Bool = false, n_steps::Int64 = 200)
    genes_alpha = map(x->borne(x,-10.0, 10.0), genes[1:32])
...
for indiv in 17:20
    display(histogram(abs.(results["weights"][indiv][33:end-1]), xlabel = "$(indiv)"))
    println("Inh weight $(indiv)= $(results["weights"][indiv][end])")
end

# %%
gr()
histogram(map(x->borne(x,-1.0,1.0),results["weights"][20][1:32]), bins = collect(-1.0:0.2:1.0))
