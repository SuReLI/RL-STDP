...
function run_sim(net::Network, n_steps::Int64, render::Bool=false;
    ...
    @inbounds for j in 1:n_steps
        ...
        spikes = latency_spikes(obs, cells_pos, cells_vel, win_size)
        influence = 1
        @inbounds for msec in 1:win_size
            (msec==10) ? train=true : train=false
            spiked = step!(net, spikes[msec], train)
...
function randomize!(net::Network)
    m = net.param["wmax"]/2
    std = net.param["wmax"]/10
    net.s_ee .= randn(net.n_exc, net.n_exc)*std .+ m
    net.s_ee .= clamp.(net.s_ee, 0.0, net.param["wmax"])
    net.s_ee .*= net.connection_ee
end

function suma(arch::Array{Int64}, i::Int64)
    return sum(arch[1:i])
end

function randomize_layer!(net::Network)
    a = net.arch
    m_base = net.param["wmax"]/2
    std_base = net.param["wmax"]/10
    con_base = a[1]*a[2]
    for l in 1:length(a)-1
        n_con = a[l]*a[l+1]
        ratio = n_con/con_base
        m = m_base/(ratio)^0.5
        std = std_base/(ratio)^0.5
        net.s_ee[suma(a,l-1)+1:suma(a,l),suma(a,l)+1:suma(a,l+1)] .= randn(a[l], a[l+1])*std .+ m
    end
    net.s_ee .= clamp.(net.s_ee, 0.0, net.param["wmax"])
    net.s_ee .*= net.connection_ee
end
...
@time spike_history, reward = run_sim(n, 200, method = "snn")
push!(rewards, reward)
...
layer_spike_hist(n.arch, spike_history, 4)
